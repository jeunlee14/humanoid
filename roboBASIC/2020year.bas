...
DIM 전진 AS BYTE
DIM 문열기 AS BYTE
DIM 이동방향 AS BYTE '오른쪽 = 1, 왼쪽 = 2, 앞쪽 = 3, 뒤쪽 = 4
DIM 화살표 AS BYTE '오른쪽 = 1, 왼쪽 = 2
DIM 알파벳 AS BYTE 'A = 1, B = 2, C = 3, D = 4
DIM 알파벳색상 AS BYTE '빨강 = 1, 파랑 = 2
DIM 확인구역수 AS BYTE
DIM 구역 AS BYTE '안전구역 = 1, 확진구역 = 2
DIM 확진구역1 AS BYTE 'A = 1, B = 2, C = 3, D = 4
DIM 확진구역2 AS BYTE 'A = 18 B = 2, C = 3, D = 4
DIM 확진구역3 AS BYTE 'A = 1, B = 2, C = 3, D = 4
DIM 확인 AS BYTE
DIM 회전수 AS BYTE
DIM 결과 AS BYTE

DIM cnt AS INTEGER
DIM cnt_turn AS INTEGER
DIM cnt_turn_div AS INTEGER
DIM numY AS BYTE
...
라인트레이싱:
    IF A > 200 THEN '코너인식
        ...
        ELSEIF A = 230 THEN '오른쪽20
            이동방향 = 1
            GOSUB 오른쪽옆으로20
        ELSEIF A = 228 THEN '왼쪽20
            이동방향 = 2
            GOSUB 왼쪽옆으로20
        ELSEIF A = 231 THEN '후진종종
            이동방향 = 4
            GOSUB 후진종종걸음
        ELSEIF A = 221 THEN '전진종종
            이동방향 = 3
            GOSUB 전진종종걸음

        ELSEIF A = 219 THEN '오른쪽턴10
            이동방향 = 1
            GOSUB 오른쪽턴10
        ELSEIF A = 225 THEN '왼쪽턴10
            이동방향 = 2
            GOSUB 왼쪽턴10
...
라인트레이싱_코너:
        IF 동작모드_old = 2 OR 동작모드_old = 10 THEN '코너
            GOTO 모드_알파벳색상인식
        ELSEIF 동작모드_old = 6 OR 동작모드_old = 9 THEN '문 앞
            GOTO 라인트레이싱_문앞	
        ELSE '임시!!!!!!!!!!!!!!!!!!!!!!!
            GOTO RX_EXIT
...
라인트레이싱_문앞:
    ...
    ELSEIF A = 130 THEN '오른쪽20
        이동방향 = 1
        GOSUB 오른쪽옆으로20
    ELSEIF A = 128 THEN '왼쪽20
        이동방향 = 2
        GOSUB 왼쪽옆으로20
    ELSEIF A = 131 THEN '후진종종
        이동방향 = 4
        GOSUB 후진종종걸음
    ELSEIF A = 121 THEN '전진종종
        이동방향 = 3
        보행횟수 = 3
        GOSUB 횟수_전진종종걸음
    ...
    ELSEIF A = 119 THEN '오른쪽턴10
        이동방향 = 1
        GOSUB 오른쪽턴10
    ELSEIF A = 125 THEN '왼쪽턴10
        이동방향 = 2
        GOSUB 왼쪽턴10

    ELSE '인식실패 (A = 129)
        IF 이동방향 = 1 THEN '이전 이동방향이 오른쪽
            GOSUB 왼쪽옆으로20
        ELSEIF 이동방향 = 2 THEN '이전 이동방향이 왼쪽
            GOSUB 오른쪽옆으로20
        ELSEIF 이동방향 = 3 THEN '이전 이동방향이 앞쪽
            GOSUB 후진종종걸음
        ELSEIF 이동방향 = 4 THEN '이전 이동방향이 뒤쪽
            GOSUB 전진종종걸음
        ELSE '이전 이동방향이 없으면
            GOSUB 전진종종걸음
